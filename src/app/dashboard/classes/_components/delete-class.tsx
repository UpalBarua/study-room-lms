...
export function DeleteClass({ currentClass }: Readonly<DeleteClassProps>) {
  const [isOpen, setIsOpen] = useState(false);
  const [isLoading, startTransition] = useTransition();

  function onClick() {
    startTransition(async () => {
      const response = await deleteClassAction({ uuid: currentClass.uuid });
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Are you sure to delete "{currentClass.name}"?
          </AlertDialogTitle>
          <AlertDialogDescription>
